...
async def notifier(queue: asyncio.Queue, type: str):
...
async def timer(game: Game, secs: int = round_length):
    ...
    scheduler.add_job(
        func=notifier, trigger=DateTrigger(half_time), args=[queue, "half_time"]
    )
    scheduler.add_job(
        func=notifier, trigger=DateTrigger(hurry_up_time), args=[queue, "hurry_up"]
    )
    scheduler.add_job(func=notifier, trigger=DateTrigger(end_time), args=[queue, "end"])
    await queue.get()
    if not game.exists():
        return
    ...
    for user in game.active_users():
        await bot.send_message(
            user.id, messager.round_end(round), parse_mode="MarkdownV2"
        )
    if round == 1:
        css_generator(game)
    html_page_generator(game)
    with ZipFile(
        f"./round results/{game.id} game {round} round results.zip", "w"
    ) as zfile:
    ...
    if round == 6:
        ...
        for user in game.active_users():
            await bot.send_message(
                user.id, messager.end_of_the_game(), parse_mode="MarkdownV2"
            )
...
async def print_all_info(nround: int, planet: Planet, userid: int):
    await bot.send_message(
        userid, messager.round_message(nround), parse_mode="MarkdownV2"
    )
    city_msg = await bot.send_message(
        userid,
        messager.city_stats_message(planet),
...
async def start(message: types.Message):
    ...
    if user is None:
        ...
        await message.answer(
            messager.start_msg(
                isadmin, user.game() is not None, message.from_user.first_name
            ),
...
@dp.message(
    Command("request"),
    F.chat.func(lambda user: not User(user.id, db_connection).is_admin()),
)
async def request(message: types.Message):
    ...
    await bot.send_message(
        OwnerID,
        messager.request_for_leader(
            f"[{message.from_user.full_name}](tg://user?id={userid})"
        ),
        reply_markup=kb.request_keyboard(userid),
        parse_mode="markdownv2",
    )
...
@dp.message(
    (F.text == "Войти в игру")
    & F.chat.func(lambda user: User(user.id, db_connection).is_admin())
)
...
async def leave_lobby(message: types.Message):
    ...
    res = await method_executor_msg(
        user.kick_user, tgid, reply_markup=kb.start_keyboard(user.is_admin())
    )
    if res and game is not None:
        if not user.is_admin():
            await message.answer(
                messager.leaving_msg(), reply_markup=kb.start_keyboard(user.is_admin())
            )
                ...
                for ouser in active_users:
                    await bot.send_message(
                        ...
                        messager.leave_for_others(
                            planet.name(), len(active_users), num_users
                        ),
                    )
        else:
            await message.answer(
...
async def create_game(message: types.Message, state: FSMContext):
    await message.answer(
        "Выберите набор планет и городов для игры", reply_markup=kb.pack_keyboard()
    )
...
async def start_game(message: types.Message):
    ...
    if game is None:
        await message.answer(
            messager.starting_game_not_being_in(), reply_markup=kb.start_keyboard(True)
        )
        ...
        await message.answer(
            messager.not_enough_players(
                len(game.active_users()), game.number_of_planets()
            )
        )
...
async def end_negotiations(call: types.CallbackQuery, state: FSMContext):
    ...
    if res:
        for admin in game.admins_list():
            await bot.send_message(
                admin.id, messager.negotiations_ended_admin(planet.name())
            )
...
async def ingame_action(call: types.CallbackQuery, state: FSMContext):
    ...
    if args[0] == "accept" or args[0] == "deny":
        ...
        if args[0] == "accept":
            res = await method_executor_call(
                planet.accept_diplomatist_from, call, from_planet
            )
            if res:
                call.answer("")
                msg = await message.answer(
                    messager.wait_for_diplomatist(from_planet.name()),
                    reply_markup=kb.end_negotiations_keyboard,
                )
                cursor.execute(
                    "UPDATE PlanetMessages SET messageid=%s WHERE messageid=%s AND mtype='Negotiations'",
                ...
                await bot.send_message(
                    from_planet.user_id(), messager.negotiations_accepted(planet.name())
                )
        ...
        # Denying of negotiations
            ...
            call.answer("")
            await bot.send_message(
                from_planet.user_id(), messager.negotiations_denied(planet.name())
            )
            cursor.execute(
                "DELETE FROM PlanetMessages WHERE messageid=%s AND mtype='Negotiations'",
                (message.message_id,),
            )
            db_connection.commit()
            await bot.delete_message(id, message.message_id)
        ...
        if res:
            ...
            await message.edit_reply_markup(
                reply_markup=kb.sanctions_keyboard(
                    planet, game.planets(), planet.ordered_sanctions_list()
                )
            )
        ...
        if res:
            ...
            await message.edit_reply_markup(
                reply_markup=kb.invent_meteorites_keyboard(
                    planet, planet.is_invent_in_order()
                )
            )
        ...
        if res:
            ...
            await message.edit_reply_markup(
                reply_markup=kb.meteorites_keyboard(
                    planet, planet.number_of_ordered_meteorites()
                )
            )
        ...
        if other_user_id is None:
            ...
            cursor.execute(
                "INSERT INTO PlanetMessages(ownerid, planetid, messageid, mtype) VALUES (%s, %s, %s, 'Negotiations')",
        ...
        await state.update_data(
            {"from_planet": planet, "to_planet": other_planet, "game_id": game.id}
        )
    ...
    if old_balance != new_balance:
        cursor.execute(
            "SELECT id FROM InfoMessages WHERE planetid=%s AND mtype='City'",
            (planet.id,),
        )
    ...
    if old_num_meteorites != new_num_meteorites:
        cursor.execute(
            "SELECT id FROM InfoMessages WHERE planetid=%s AND mtype='Meteorites'",
            (planet.id,),
        )
        msgid = cursor.fetchone()[0]
...
async def set_amount_of_money(message: types.Message, state: FSMContext):
    ...
    res = await method_executor_msg(
        from_planet.transfer, message.from_user.id, to_planet.id, amount
    )
    if res:
        nround = game.show_round()
        cursor.execute(
        ...
        from_msg = await bot.edit_message_text(
            messager.city_stats_message(from_planet),
            chat_id=from_planet.user_id(),
        ...
        to_msg = await bot.edit_message_text(
            messager.city_stats_message(to_planet),
            chat_id=to_planet.user_id(),
        ...
        cursor.execute(
            "UPDATE InfoMessages SET id=%s WHERE id=%s",
            (from_msg.message_id, from_city_id),
        )
        cursor.execute(
            "UPDATE InfoMessages SET id=%s WHERE id=%s", (to_msg.message_id, to_city_id)
        )
        db_connection.commit()
