...
def sanctions_keyboard(
    planet: Planet, planets: list[Planet], under_sanctions: list[Planet]
) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
...
def invent_meteorites_keyboard(planet: Planet, chosen: bool) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=("✅ " if chosen else "") + "Разработать (500 💵)",
...
def meteorites_keyboard(planet: Planet, chosen: int) -> InlineKeyboardMarkup:
    ...
    for i in range(1, 4):
        if chosen == i:
            ...
            builder.add(
                InlineKeyboardButton(
                    text=f"{i} ({150 * i} 💵)", callback_data=f"create {planet.id} {i}"
                )
            )
...
def other_planets_keyboard(
    nround: int, planet: Planet, other_planet: Planet, chosen_cities: list[City]
) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    if nround > 1:
