...
def start_city_keyboard(cities: list[City], developed: list[City]) -> InlineKeyboardMarkup:
    ...
    for city in cities:
        ...
        kb.add(InlineKeyboardButton(text=f'{str1}📈 {city.name()} (150 💵)', callback_data=f'develop {city.name()}'))
    return kb
...
def city_keyboard(cities: list[City], under_shield: list[City], developed: list[City]) -> InlineKeyboardMarkup:
    ...
    for city in cities:
        ...
        if city in under_shield:
            str2 = '✅'
        kb.add(InlineKeyboardButton(text=f'{str1}📈 {city.name()} (150 💵)', callback_data=f'develop {city.name()}'), InlineKeyboardButton(text=f'{str2}🛡️ {city.name()} (300 💵)', callback_data=f'defend {city.name()}'))
    return kb
...
def invent_meteorites_keyboard(chosen: bool) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton(text=('✅ ' if chosen else '') + 'Разработать (500 💵)', callback_data='invent'))

def meteorites_keyboard(chosen: int) -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(row_width=3)
    for i in range(1, 4):
        if chosen == i:
            kb.add(InlineKeyboardButton(text=f'✅ Купить {i} ({150*i} 💵)', callback_data=f'create 0'))
        else:
            kb.add(InlineKeyboardButton(text=f'Купить {i} ({150*i} 💵)', callback_data=f'create {i}'))
    return kb
