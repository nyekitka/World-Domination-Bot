...
@lobby_router.message(ReplyButtonFilter('Создать лобби'), AdminFilter())
async def create_game(
    ...
    await message.answer(
        **renderer.render('choose_pack'), reply_markup=kb.pack_keyboard()
    )
...
async def set_number_of_planets(
    ...
    game = await game_client.create_game(
        session, admin_id=call.from_user.id, pack=pack, number_of_planets=number
    )
    await call.answer()
    await call.message.answer(
...
@lobby_router.message(ReplyButtonFilter('Войти в лобби'))
async def enter_game_player(
    ...
    user = await user_client.make_new_user_if_not_exists(
        session, message.from_user.id, False
    )
...
async def leave_lobby(
    ...
    logger.info(
        'lobby_router.leave_lobby: User id=%s is leaving a lobby', message.from_user.id
    )
...
async def chosen_lobby_admin(
    ...
    res = await method_executor_call(
        user_client.join_user, call, renderer, session, tgid, game.id
    )
    if not res:
        return
    await call.message.answer(
...
async def chosen_lobby(
    ...
    res = await method_executor_call(
        user_client.join_user, call, renderer, session, tg_id, game.id
    )
    if not res:
        return
