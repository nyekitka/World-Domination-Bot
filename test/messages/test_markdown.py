...
def test_start_round_for_players_is_valid_markdown(
    renderer_ru, renderer_en, game, round
):
    game.round = round
...
@pytest.mark.parametrize('round', (1, 2))
def test_start_round_for_admins_is_valid_markdown(
    renderer_ru, renderer_en, game, round
):
    game.round = round
...
def test_common_planet_info_is_valid_markdown(renderer_ru, renderer_en, planet, cities):
    assert_valid_markdown_v2(
        renderer_ru.render('common_planet_info', planet=planet, cities=cities)
    )
    assert_valid_markdown_v2(
        renderer_en.render('common_planet_info', planet=planet, cities=cities)
    )
...
def test_sanctions_info_is_valid_markdown(renderer_ru, renderer_en, sanctioned_planets):
    assert_valid_markdown_v2(
        renderer_ru.render('sanctions_info', sanctioned_planets=sanctioned_planets)
    )
    assert_valid_markdown_v2(
        renderer_en.render('sanctions_info', sanctioned_planets=sanctioned_planets)
    )
...
def test_meteorites_info_not_invented_is_valid_markdown(
    renderer_ru, renderer_en, planet_not_invented
):
    assert_valid_markdown_v2(
        renderer_ru.render('meteorites_info', planet=planet_not_invented)
    )
    assert_valid_markdown_v2(
        renderer_en.render('meteorites_info', planet=planet_not_invented)
    )
...
def test_other_planet_info_is_valid_markdown(renderer_ru, renderer_en, planet, cities):
    assert_valid_markdown_v2(
        renderer_ru.render('other_planet_info', planet=planet, cities=cities)
    )
    assert_valid_markdown_v2(
        renderer_en.render('other_planet_info', planet=planet, cities=cities)
    )
...
def test_request_notification_for_leader_is_valid_markdown(
    renderer_ru, renderer_en, user_ru, user_en
):
    assert_valid_markdown_v2(
        renderer_ru.render('request_notification_for_leader', user=user_ru)
    )
    assert_valid_markdown_v2(
        renderer_en.render('request_notification_for_leader', user=user_en)
    )
